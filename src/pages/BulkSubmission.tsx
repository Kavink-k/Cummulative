...
import { toast } from "sonner";
import { uploadBulkFile } from "../lib/api"; // API upload function
...
export default function BulkSubmission() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.error || "Bulk upload failed. Try again.");
      console.error("Bulk upload error:", error);
    } finally {
      setIsLoading(false);
    }
  ...
  const downloadTemplate = () => {
    ...
    link.href = "/template/CummulativeBulkTemplate.xlsx";   // store file here: public/template/student_template.xlsx
    link.download = "CummulativeBulkTemplate.xlsx";
    link.click();
  };
        ...
        <div className="px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Bulk Submission</h1>
              <p className="text-sm text-muted-foreground">
                Upload multiple student records using CSV or Excel file.
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="border-2 shadow-lg">
          ...
          <CardContent className="pt-6 space-y-4">
            ...
            <ul className="text-sm text-muted-foreground list-disc pl-5 space-y-1">
              ...
              <li>Fill all required fields (Name, RegNo, DOB, Program, etc.).</li>
              <li>Ensure there are no blank rows.</li>
              <li>Template is available for correct format reference.</li>
            </ul>
          </CardContent>
        </Card>
      </div>
